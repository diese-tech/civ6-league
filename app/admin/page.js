                ...
                {editRule?.id === rule.id ? (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex gap-2 shrink-0">
                      ...
                      <button onClick={() => delRule(rule.id)} className="px-3 py-1 border border-[var(--red)]/30 text-[var(--red)] font-condensed text-[11px] tracking-wider uppercase rounded-md hover:bg-[var(--red)]/10">Delete</button>
                    </div>
                  </div>
                )}
              </div>
...
export default function AdminPage() {
  ...
  const handleDelete = async (type, id, label) => {
    ...
    try {
      const res = await fetch("/api/admin", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type, id }) });
  ...
  const openEdit = (type, item) => {
    ...
    if (type === "season") {
      setEditForm({ name: item.name || "", startDate: item.startDate?.split("T")[0] || "", endDate: item.endDate?.split("T")[0] || "", isActive: item.isActive || false });
    } else if (type === "player") {
      setEditForm({ username: item.username || "", eloRating: item.eloRating || 1500, division: item.division || "Prince", wins: item.wins || 0, losses: item.losses || 0 });
    }
  };

  const saveEdit = async () => {
    try {
      const res = await fetch("/api/admin", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: editModal.type, id: editModal.item.id, data: editForm }) });
  ...
  return (
    <section className="max-w-7xl mx-auto px-6 py-12">
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
        <div><h1 className="font-display text-3xl font-bold">Admin Panel</h1><div className="w-10 h-0.5 bg-gold mt-2" /></div>
        <span className="px-3 py-1 rounded-full text-[11px] font-condensed font-semibold tracking-wider uppercase bg-gold/15 text-gold border border-gold/25">Admin Access</span>
      </div>
      ...
      {tab === "players" && (
        <div className="fade-in">
          <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-5 mb-6">
            <p className="text-sm text-[var(--text-secondary)]">Players are added automatically via the Discord bot. Edit or remove players here.</p>
          </div>
          <div className="overflow-x-auto rounded-xl border border-[var(--border)]">
            <table className="w-full border-collapse">
              <thead><tr>{["Username", "Discord ID", "Division", "Rating", "W/L", "Actions"].map((h) => (<th key={h} className="px-4 py-3 text-left font-condensed text-[11px] font-semibold tracking-[2px] uppercase text-[var(--text-muted)] bg-[var(--bg-secondary)] border-b border-[var(--border)]">{h}</th>))}</tr></thead>
              <tbody>
                {players.map((p) => (
                  <tr key={p.id} className="hover:bg-gold/[0.02]">
                    ...
                    <td className="px-4 py-3 font-mono text-sm border-b border-[var(--border)]"><span className="text-[var(--green)]">{p.wins}</span>/<span className="text-[var(--red)]">{p.losses}</span></td>
                    <td className="px-4 py-3 border-b border-[var(--border)]">
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => handleDelete("player", p.id, p.username)} className={`px-3 py-1 font-condensed text-[11px] tracking-wider uppercase rounded-md transition-all ${confirmDelete === `player-${p.id}` ? "bg-[var(--red)] text-white" : "border border-[var(--red)]/30 text-[var(--red)] hover:bg-[var(--red)]/10"}`}>{confirmDelete === `player-${p.id}` ? "Confirm?" : "Delete"}</button>
                      </div>
                    </td>
                  </tr>
                ))}
                {players.length === 0 && <tr><td colSpan={6} className="px-4 py-12 text-center text-[var(--text-muted)]">No players yet.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div className="fade-in">
          <div className="overflow-x-auto rounded-xl border border-[var(--border)]">
            <table className="w-full border-collapse text-sm">
              <thead><tr>{["ID", "Player 1", "Player 2", "Result", "Date", "Actions"].map((h) => (<th key={h} className="px-3 py-2.5 text-left font-condensed text-[10px] font-semibold tracking-[2px] uppercase text-[var(--text-muted)] bg-[var(--bg-secondary)] border-b border-[var(--border)]">{h}</th>))}</tr></thead>
              <tbody>
                {matches.map((m) => (
                  <tr key={m.id} className="hover:bg-gold/[0.02]">
                    ...
                    <td className="px-3 py-2 border-b border-[var(--border)]">
                      <button onClick={() => handleDelete("match", m.id, `match #${m.id}`)} className={`px-3 py-1 font-condensed text-[10px] tracking-wider uppercase rounded-md transition-all ${confirmDelete === `match-${m.id}` ? "bg-[var(--red)] text-white" : "border border-[var(--red)]/30 text-[var(--red)] hover:bg-[var(--red)]/10"}`}>{confirmDelete === `match-${m.id}` ? "Confirm?" : "Delete"}</button>
                    </td>
                  </tr>
                ))}
                {matches.length === 0 && <tr><td colSpan={6} className="px-3 py-12 text-center text-[var(--text-muted)]">No matches yet.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
              ...
              <div key={s.id} className={`bg-[var(--bg-card)] border rounded-xl p-5 flex items-center justify-between flex-wrap gap-4 ${s.isActive ? "border-gold-dim" : "border-[var(--border)]"}`}>
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-condensed font-semibold tracking-wider uppercase ${st.cls}`}>{st.label}</span>
                  </div>
                  <div className="font-display text-base font-semibold">{s.name}</div>
                  <div className="font-mono text-[11px] text-[var(--text-muted)]">{new Date(s.startDate).toLocaleDateString()} — {new Date(s.endDate).toLocaleDateString()}</div>
                </div>
                <div className="flex gap-2">
                  ...
                  <button onClick={() => handleDelete("season", s.id, s.name)} className={`px-4 py-2 font-condensed text-[11px] tracking-wider uppercase rounded-md transition-all ${confirmDelete === `season-${s.id}` ? "bg-[var(--red)] text-white" : "border border-[var(--red)]/30 text-[var(--red)] hover:bg-[var(--red)]/10"}`}>{confirmDelete === `season-${s.id}` ? "Confirm?" : "Delete"}</button>
                </div>
              </div>
            );
          })}
      ...
      {tab === "announcements" && (
        <div className="fade-in">
          <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-5 mb-6">
            <p className="text-sm text-[var(--text-secondary)]">Announcements are posted via Discord <code className="text-gold">.announce</code> or the website <code className="text-gold">/blog/new</code>. Delete them here.</p>
          </div>
          <div className="space-y-3">
            {announcements.map((a) => (
          ...
          <div className="bg-[var(--bg-secondary)] border border-[var(--border)] rounded-xl p-8 max-w-md w-full fade-in" onClick={(e) => e.stopPropagation()}>
            {editModal.type === "season" && (
              <>
                ...
                <div className="mb-4"><label className={labelCls}>Season Name</label><input className={inputCls} value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} /></div>
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div><label className={labelCls}>Start Date</label><input type="date" className={inputCls} value={editForm.startDate} onChange={(e) => setEditForm({ ...editForm, startDate: e.target.value })} /></div>
                  <div><label className={labelCls}>End Date</label><input type="date" className={inputCls} value={editForm.endDate} onChange={(e) => setEditForm({ ...editForm, endDate: e.target.value })} /></div>
                </div>
                <label className="flex items-center gap-3 mb-6 cursor-pointer"><input type="checkbox" checked={editForm.isActive} onChange={(e) => setEditForm({ ...editForm, isActive: e.target.checked })} className="w-4 h-4 accent-[var(--gold)]" /><span className="text-sm text-[var(--text-secondary)]">Set as active season</span></label>
              </>
            )}
            {editModal.type === "player" && (
              <>
                <h2 className="font-display text-xl text-gold mb-6">Edit Player</h2>
                <div className="mb-4"><label className={labelCls}>Username</label><input className={inputCls} value={editForm.username} onChange={(e) => setEditForm({ ...editForm, username: e.target.value })} /></div>
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div><label className={labelCls}>Rating</label><input type="number" className={inputCls} value={editForm.eloRating} onChange={(e) => setEditForm({ ...editForm, eloRating: e.target.value })} /></div>
                  <div><label className={labelCls}>Division</label><select className={inputCls} value={editForm.division} onChange={(e) => setEditForm({ ...editForm, division: e.target.value })}>{["Deity", "Immortal", "Emperor", "King", "Prince"].map((d) => <option key={d} value={d}>{d}</option>)}</select></div>
                </div>
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div><label className={labelCls}>Wins</label><input type="number" className={inputCls} value={editForm.wins} onChange={(e) => setEditForm({ ...editForm, wins: e.target.value })} /></div>
                  <div><label className={labelCls}>Losses</label><input type="number" className={inputCls} value={editForm.losses} onChange={(e) => setEditForm({ ...editForm, losses: e.target.value })} /></div>
                </div>
              </>
            )}
